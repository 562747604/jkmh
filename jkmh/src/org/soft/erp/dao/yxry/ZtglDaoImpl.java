 ...
 * @version V1.0   
 */
public class ZtglDaoImpl {
	String tblnameString="d_ztmb";
	// 分页动态查询
	...
	// 动态插入
	public String insert(final Ztgl bean) {
		...
		String sql = new SQL() {
			{
				...
				if(bean.getNr() != null && !bean.getNr().equals("")){
					VALUES("nr", "#{nr}");
				}
				if(bean.getCreator() != null && !bean.getCreator().equals("")){
					VALUES("creator", "#{creator}");
				}
				if(bean.getCreatorname() != null && !bean.getCreatorname().equals("")){
					VALUES("creatorname", "#{creatorname}");
				}
				if(bean.getMmc() != null && !bean.getMmc().equals("")){
					VALUES("mmc", "#{mmc}");
				}	
				if(bean.getFs() != null && !bean.getFs().equals("")){
					VALUES("fs", "#{fs}");
				}		
	...
	// 动态更新
	public String update(final Ztgl bean) {
		String sql = new SQL() {
			{
