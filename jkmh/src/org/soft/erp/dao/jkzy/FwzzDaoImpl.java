...
public class FwzzDaoImpl {
	...
	// 动态更新
	public String updateJkzj(final String id,final String zyid,final String zyname) {
		String sql = new SQL() {
			{
				UPDATE(tblnameString);
				if(zyid != ""){
	...
	// 动态更新
	public String updateJkzjzt(final String id) {
		String sql = new SQL() {
			{
				UPDATE(tblnameString);
				SET("zjshzt = '已审核'");
		...
		String sql = new SQL() {
			{
				...
				for(int h=0;h<rs.getRowCount();h++){
					...
					VALUES("dw", "'"+ row.get("dw")+"'");
					VALUES("sfzh","'"+  row.get("sfzh")+"'");
					VALUES("sjh","'"+  row.get("mobile")+"'");
					VALUES("email","'"+  row.get("yx")+"'");
					VALUES("creator","'"+  zjname+"'");
					VALUES("tjh", "'"+ row.get("tjh")+"'");
					VALUES("fczt", "'未完成'");
